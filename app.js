...
let state = { items: [], fileName: "", updatedAt: null };
...
const mapPlating = document.getElementById("mapPlating");
const mapMaterial = document.getElementById("mapMaterial");
const mapColorEn = document.getElementById("mapColorEn");
const mapColorCn = document.getElementById("mapColorCn");
...
const resultDetailsRow = document.getElementById("resultDetailsRow");
const resultPlating = document.getElementById("resultPlating");
const resultPlatingRow = document.getElementById("resultPlatingRow");
const resultMaterial = document.getElementById("resultMaterial");
const resultMaterialRow = document.getElementById("resultMaterialRow");
const resultColor = document.getElementById("resultColor");
const resultColorRow = document.getElementById("resultColorRow");
...
function normalizeStyle(v) { return String(v ?? "").trim().toUpperCase(); }
function formatPrice(v) {
  const n = Number(String(v).replace(/[^0-9.\-]/g, ""));
  if (!isNaN(n) && String(v).match(/[0-9]/)) return "$" + n.toFixed(2);
  return String(v ?? "—");
}
...
function loadData() {
  try {
    ...
    buildMaps();
    return true;
  } catch (e) { console.error("Failed to load stored data", e); return false; }
}

function saveData() { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); }

function loadScanLog() {
  try { const raw = localStorage.getItem(SCAN_LOG_KEY); scanLog = raw ? JSON.parse(raw) : []; }
  catch (e) { scanLog = []; }
}
function saveScanLog() { localStorage.setItem(SCAN_LOG_KEY, JSON.stringify(scanLog)); }
function logScan(item) {
  scanLog.push({ time: new Date().toISOString(), barcode: item.barcode || "", style: item.style || "", price: item.price ?? "", desc: item.desc || "" });
  saveScanLog();
}
...
function init() {
  loadScanLog();
  if (loadData()) { renderMainStats(); showScreen("main"); } else { showScreen("empty"); }
}
init();
...
function handleFile(e) {
  ...
  reader.onload = (evt) => {
    ...
    } catch (err) { console.error(err); toast("Couldn't read that file. Try exporting as .xlsx or .csv."); }
  };
...
/* Some catalogs have a title/date row (or several) before the real header row.
   Scan the first 15 rows for the one that looks most like a header: mostly
   short text cells, few/no numbers, and reasonably many non-empty cells. */
function detectHeaderRow(rows) {
  ...
  for (let i = 0; i < scanLimit; i++) {
    ...
    const shortTextCount = cells.filter(c => c.length > 0 && c.length <= 12 && !/^-?\d+(\.\d+)?$/.test(c)).length;
    const score = cells.length + shortTextCount * 2 - numericCount * 3;
...
function openMappingScreen(rows) {
  ...
  const headers = rows[headerRowIndex].map(h => String(h ?? "").trim() || "(blank)");
  const dataRows = rows.slice(headerRowIndex + 1, headerRowIndex + 6);

  [mapBarcode, mapStyle, mapPrice, mapDesc, mapSilver, mapWeight, mapDetails, mapPlating, mapMaterial, mapColorEn, mapColorCn]
  ...
  optionsFor(mapDetails, true);
  optionsFor(mapPlating, true);
  optionsFor(mapMaterial, true);
  optionsFor(mapColorEn, true);
  optionsFor(mapColorCn, true);
  ...
  const bIdx = guess(["barcode", "upc", "ean", "sku", "条码"]);
  const sIdx = guess(["style", "item no", "item number", "model", "产品编号", "货号"]);
  const silverIdxGuess = guess(["silver"]);
  const weightIdxGuess = guess(["weight", "重量"], ["银重"]);
  const pIdx = guess(["price", "cost", "wholesale", "retail", "售价"], ["silver", "银"]);
  const dIdx = guess(["desc", "name", "title", "product_type", "product type", "type", "详细描述"]);
  const detailsIdx = guess(["尺寸", "size", "details"]);
  const platingIdx = guess(["电镀", "plating"]);
  const materialIdx = guess(["材质", "material"]);
  const colorHits = headers.reduce((acc, h, i) => { if (guess(["颜色", "color"]) === i || h.toLowerCase().includes("颜色") || h.toLowerCase().includes("color")) acc.push(i); return acc; }, []);
  const colorEnGuess = colorHits.find(i => /[a-zA-Z]/.test(headers[i])) ?? colorHits[0];
  const colorCnGuess = colorHits.find(i => i !== colorEnGuess && /[\u4e00-\u9fff]/.test(headers[i])) ?? colorHits[1];

  if (bIdx >= 0) mapBarcode.value = String(bIdx);
  if (sIdx >= 0) mapStyle.value = String(sIdx); else mapStyle.value = "0";
  ...
  mapDetails.value = detailsIdx >= 0 ? String(detailsIdx) : "-1";
  mapPlating.value = platingIdx >= 0 ? String(platingIdx) : "-1";
  mapMaterial.value = materialIdx >= 0 ? String(materialIdx) : "-1";
  mapColorEn.value = colorEnGuess !== undefined ? String(colorEnGuess) : "-1";
  mapColorCn.value = colorCnGuess !== undefined ? String(colorCnGuess) : "-1";

  const thead = previewTable.querySelector("thead");
  const tbody = previewTable.querySelector("tbody");
  thead.innerHTML = "<tr>" + headers.map(h => `<th>${escapeHtml(h)}</th>`).join("") + "</tr>";
  tbody.innerHTML = dataRows.map(r => "<tr>" + headers.map((_, i) => `<td>${escapeHtml(String(r[i] ?? ""))}</td>`).join("") + "</tr>").join("");
...
confirmMapping.addEventListener("click", () => {
  ...
  const detailsIdx = Number(mapDetails.value);
  const platingIdx = Number(mapPlating.value);
  const materialIdx = Number(mapMaterial.value);
  const colorEnIdx = Number(mapColorEn.value);
  const colorCnIdx = Number(mapColorCn.value);
  ...
  for (const row of dataRows) {
    ...
    if (!barcode) continue;
    const colorEn = colorEnIdx >= 0 ? String(row[colorEnIdx] ?? "").trim() : "";
    const colorCn = colorCnIdx >= 0 ? String(row[colorCnIdx] ?? "").trim() : "";
    const color = [colorEn, colorCn].filter(Boolean).join(" / ");
    items.push({
      barcode,
      ...
      details: detailsIdx >= 0 ? String(row[detailsIdx] ?? "").trim() : "",
      plating: platingIdx >= 0 ? String(row[platingIdx] ?? "").trim() : "",
  ...
  state = { items, fileName: pendingFileName, updatedAt: new Date().toISOString() };
  saveData();
  buildMaps();
  renderMainStats();
  rawRows = null;
...
settingsBtn.addEventListener("click", () => {
  ...
  const cardInfo = cards.length
    ? ` ${cards.length} business card${cards.length === 1 ? "" : "s"} saved.`
    : " No business cards saved yet.";
  settingsInfo.textContent = fileInfo + scanInfo + cardInfo;
  settingsModal.hidden = false;
});
closeSettingsBtn.addEventListener("click", hideSettings);
function hideSettings() { settingsModal.hidden = true; }

replaceFileBtn.addEventListener("click", () => { hideSettings(); fileInput.click(); });
...
exportScansBtn.addEventListener("click", async () => {
  if (!scanLog.length) { toast("No scanned items yet."); return; }
  try {
    const rows = scanLog.map(s => ({ Barcode: s.barcode, Style: s.style, Price: s.price, Description: s.desc, "Scanned At": new Date(s.time).toLocaleString() }));
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ files: [file], title: "Scanned items", text: `${scanLog.length} scanned item${scanLog.length === 1 ? "" : "s"} — attach this file to your email.` });
    } else {
  ...
  } catch (e) { console.error(e); toast("Couldn't export right now."); }
});

exportCardsBtn.addEventListener("click", async () => {
  if (!cards.length) { toast("No business cards saved yet."); return; }
  try {
    const rows = cards.map(c => ({ Name: c.name, Company: c.company, Title: c.title, Phone: c.phone, Email: c.email, Notes: c.notes, "Saved At": new Date(c.savedAt).toLocaleString() }));
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    ...
    const blob = new Blob([wbout], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-");
    const filename = `business-cards-${stamp}.xlsx`;
    const file = new File([blob], filename, { type: blob.type });
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ files: [file], title: "Business cards", text: `${cards.length} business card${cards.length === 1 ? "" : "s"} — attach this file to your email.` });
    } else {
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = filename;
...
function setRow(rowEl, valEl, value) {
  if (value) { valEl.textContent = value; rowEl.style.display = "flex"; }
...
function showResult(item, query) {
  ...
  if (item) {
    ...
    setRow(resultColorRow, resultColor, item.color);
    setRow(resultDetailsRow, resultDetails, item.details);
    setRow(resultPlatingRow, resultPlating, item.plating);
    setRow(resultMaterialRow, resultMaterial, item.material);
    setRow(resultDescRow, resultDesc, item.desc);
    setRow(resultSilverRow, resultSilver, item.silver ? formatPrice(item.silver) : "");
    setRow(resultWeightRow, resultWeight, item.weight ? String(item.weight) : "");
  } else {
    ...
    [resultColorRow, resultDetailsRow, resultPlatingRow, resultMaterialRow, resultDescRow, resultSilverRow, resultWeightRow]
      .forEach(r => (r.style.display = "none"));
  }
}

function renderHistory() {
  historyEl.innerHTML = scanHistory.map(it => `<div class="history-item"><span class="h-style">${escapeHtml(it.style || it.barcode)}</span><span class="h-price">${formatPrice(it.price)}</span></div>`).join("");
}
...
async function openCamera() {
  if (!state.items.length) { toast("Load a pricing file first."); return; }
  cameraModal.hidden = false;
  try {
    if ("BarcodeDetector" in window) { usingNativeDetector = true; await startNativeDetector(); }
    else { usingNativeDetector = false; await startZXing(); }
  } catch (err) { console.error(err); toast("Couldn't access the camera. Check camera permission in your browser settings."); closeCamera(); }
}
...
async function startNativeDetector() {
  ...
  const tick = async () => {
    ...
    try { const codes = await detector.detect(video); if (codes.length) { onScanSuccess(codes[0].rawValue); return; } } catch (e) {}
...
async function startZXing() {
  ...
  await zxingReader.decodeFromConstraints(constraints, video, (result, err) => { if (result) onScanSuccess(result.getText()); });
}

function onScanSuccess(text) { closeCamera(); lookup(text); }
...
function switchMode(mode) {
  ...
  if (toCards) { showCardScreen("screen-card-list"); renderCardList(); }
}
...
function loadCards() {
  try { const raw = localStorage.getItem(CARDS_KEY); cards = raw ? JSON.parse(raw) : []; }
  catch (e) { cards = []; }
}
...
function renderCardList() {
  ...
  if (!cards.length) { emptyEl.hidden = false; listEl.innerHTML = ""; return; }
  emptyEl.hidden = true;
  listEl.innerHTML = cards.slice().reverse().map(c => `<div class="card-list-item" data-id="${c.id}"><span class="c-name">${escapeHtml(c.name || "(no name)")}</span><span class="c-sub">${escapeHtml(c.company || c.phone || c.email || "")}</span></div>`).join("");
  listEl.querySelectorAll(".card-list-item").forEach(el => { el.addEventListener("click", () => openCardDetail(el.dataset.id)); });
}
...
const CARD_ZOOM_MAX = 3;
const zoomHint = document.getElementById("zoomHint");
const cardCameraFrame = document.querySelector("#cardCaptureModal .camera-frame");

function setCardZoom(z) {
  cardZoom = Math.min(CARD_ZOOM_MAX, Math.max(CARD_ZOOM_MIN, z));
  cardVideo.style.transform = `scale(${cardZoom})`;
  zoomHint.textContent = cardZoom.toFixed(1) + "x";
  zoomHint.hidden = false;
  clearTimeout(setCardZoom._t);
  setCardZoom._t = setTimeout(() => { zoomHint.hidden = true; }, 900);
}
...
cardCameraFrame.addEventListener("touchstart", (e) => {
  if (e.touches.length === 2) { pinchStartDist = touchDist(e.touches); pinchStartZoom = cardZoom; }
}, { passive: true });
cardCameraFrame.addEventListener("touchmove", (e) => {
  if (e.touches.length === 2 && pinchStartDist) {
    e.preventDefault();
    const scale = touchDist(e.touches) / pinchStartDist;
    setCardZoom(pinchStartZoom * scale);
  }
}, { passive: false });
cardCameraFrame.addEventListener("touchend", () => { pinchStartDist = null; });

scanCardBtn.addEventListener("click", openCardCamera);
closeCardCameraBtn.addEventListener("click", closeCardCamera);
cardCaptureBtn.addEventListener("click", startCaptureCountdown);

async function openCardCamera() {
  cardCaptureModal.hidden = false;
  cardZoom = 1;
  cardVideo.style.transform = "scale(1)";
  zoomHint.hidden = true;
  updateOrientationHint();
  window.addEventListener("resize", updateOrientationHint);
  try {
    cardStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment", width: { ideal: 3840 }, height: { ideal: 2160 } } });
    cardVideo.srcObject = cardStream;
    await cardVideo.play();
    if (!cardVideo.videoWidth) {
      await new Promise(resolve => {
        const done = () => { cardVideo.removeEventListener("loadedmetadata", done); resolve(); };
        cardVideo.addEventListener("loadedmetadata", done);
        setTimeout(done, 1500);
      });
    }
  } catch (err) { console.error(err); toast("Couldn't access the camera. Check camera permission in your browser settings."); closeCardCamera(); }
}

function updateOrientationHint() {
  const hint = document.getElementById("cardCameraHint");
  const isPortrait = window.innerHeight > window.innerWidth;
  hint.textContent = isPortrait
    ? "Tip: turn your phone sideways, then fill the screen with the card"
    : "Fill the screen with the card, then tap Capture";
}
...
async function startCaptureCountdown() {
  ...
  if (!w || !h) { toast("Camera still starting up — wait a second and try again."); return; }
  cardCaptureBtn.disabled = true;
  closeCardCameraBtn.disabled = true;
  const hint = document.getElementById("cardCameraHint");
  const originalHint = hint.textContent;
  for (const n of [3, 2, 1]) {
    hint.textContent = `Hold steady… ${n}`;
    await new Promise(r => setTimeout(r, 600));
  }
  hint.textContent = originalHint;
  cardCaptureBtn.disabled = false;
  closeCardCameraBtn.disabled = false;
  await captureCardPhoto();
}

async function captureCardPhoto() {
  const w = cardVideo.videoWidth, h = cardVideo.videoHeight;
  if (!w || !h) { toast("Camera still starting up — wait a second and try again."); return; }
  const cropW = w / cardZoom;
  const cropH = h / cardZoom;
  const sx = (w - cropW) / 2;
  const sy = (h - cropH) / 2;
  cardCanvas.width = cropW; cardCanvas.height = cropH;
  const ctx = cardCanvas.getContext("2d");
  ctx.drawImage(cardVideo, sx, sy, cropW, cropH, 0, 0, cropW, cropH);
...
cardDiscardBtn.addEventListener("click", () => { showCardScreen("screen-card-list"); renderCardList(); });
...
cardSaveBtn.addEventListener("click", () => {
  ...
  if (!card.name && !card.company && !card.phone && !card.email) { toast("Add at least a name, company, phone, or email."); return; }
...
function openCardDetail(id) {
  ...
  if (!card) return;
  detailCardId = id;
  cardDetailBody.innerHTML = `${card.photo ? `<div class="card-photo-preview"><img src="${card.photo}" alt="Card photo"></div>` : ""}<div class="card-detail-name">${escapeHtml(card.name || "(no name)")}</div>${card.company ? `<div class="card-detail-company">${escapeHtml(card.company)}${card.title ? " — " + escapeHtml(card.title) : ""}</div>` : ""}${card.phone ? `<div class="card-detail-row"><span class="d-label">Phone</span><span class="d-value">${escapeHtml(card.phone)}</span></div>` : ""}${card.email ? `<div class="card-detail-row"><span class="d-label">Email</span><span class="d-value">${escapeHtml(card.email)}</span></div>` : ""}${card.notes ? `<div class="card-detail-row"><span class="d-label">Notes</span><span class="d-value">${escapeHtml(card.notes)}</span></div>` : ""}`;
  showCardScreen("screen-card-detail");
}

cardDetailBackBtn.addEventListener("click", () => { showCardScreen("screen-card-list"); renderCardList(); });
...
cardDetailShareBtn.addEventListener("click", async () => {
  ...
  if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
    try { await navigator.share({ files: [file], title: card.name || "Contact" }); return; } catch (e) {}
  }
...
function buildVCard(card) {
  return [
    "BEGIN:VCARD", "VERSION:3.0", `FN:${card.name || ""}`,
