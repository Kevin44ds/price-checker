...
function showResult(item, query) {
  ...
  if (item) {
    ...
    if (item.desc) { resultDesc.textContent = item.desc; resultDescRow.style.display = "flex"; } else { resultDescRow.style.display = "none"; }
    if (item.silver) { resultSilver.textContent = formatPrice(item.silver); resultSilverRow.style.display = "flex"; } else { resultSilverRow.style.display = "none"; }
    if (item.weight) { resultWeight.textContent = String(item.weight); resultWeightRow.style.display = "flex"; } else { resultWeightRow.style.display = "none"; }
  } else {
...
async function startCaptureCountdown() {
  ...
  if (!w || !h) {
    toast("Camera still starting up — wait a second and try again.");
    return;
  }
